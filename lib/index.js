...
FreshBooks.prototype._getData = function(xml, fn) {
  var string = xml.toString();
  var start = [ string.indexOf('<invoice_id>'), string.indexOf('<invoice_id>')+12 ];
  var end = [string.indexOf('</invoice_id>'), string.indexOf('<invoice_id>')+13]
  var invoice_id = string.substring(start[1],end[1]);
  // console.log(xml.invoice_id);
  var options = {
      host: url.parse(this.url).hostname
    , port: url.parse(this.url).port
    , path: url.parse(this.url).path
  ...
  data = [];
    ...
    res.on('end', function() {
      ...
      fs.writeFile("./pdf/"+invoice_id+"_test.pdf", data, function(err) {
        if(err) {
            return console.log(err);
        }
        else {
          console.log("The file was saved!");
          fn(err, "./pdf/"+invoice_id+"_test.pdf");
        }
      }); 
