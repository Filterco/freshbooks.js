...
FreshBooks.prototype._getData = function(xml, fn) {
  var string = xml.toString();
  var start = string.indexOf('<invoice_id>');
  var end = string.indexOf('</invoice_id>');
  var invoice_id = string.substring(start+12,end);
  ...
  data = [];
    ...
    res.on('end', function() {
      ...
      console.log("Starting to save "+appRoot+"/pdf/"+invoice_id+"_test.pdf");
      console.log(appRoot2);
      fs.writeFile(appRoot+'/pdf/'+invoice_id+'_test.pdf', data, function(err) {
        if(err) {
