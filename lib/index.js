...
FreshBooks.prototype._getData = function(xml, fn) {
  ...
  console.log(xml.invoice_id, xml.invoice_id.toString());
  var options = {
      host: url.parse(this.url).hostname
    , port: url.parse(this.url).port
    , path: url.parse(this.url).path
  ...
  data = [];
    ...
    res.on('end', function() {
      ...
      fs.writeFile("./pdf/"+xml.invoice_id.toString()+"_test.pdf", data, function(err) {
        if(err) {
            return console.log(err);
        }
        else {
          console.log("The file was saved!");
          fn(err, "./pdf/"+xml.invoice_id.toString()+"_test.pdf");
        }
      }); 
