  ...
  this._setXML("invoice.create", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        return new Error("CANNOT CREATE INVOICE: " + err);
      } else {
  ...
  this._setXML("invoice.update", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT UPDATE INVOICE: " + err));
      } else {
        self.get(self.invoice_id, fn);
      }
    });
  ...
  this._setXML("invoice.get", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT GET INVOICE: " + err));
      } else {
  ...
  this._setXML("invoice.delete", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT DELETE INVOICE: " + err));
      } else {
        fn();
      }
    });
  ...
  this._setXML("invoice.list", options, function(xml) {
    ...
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT LIST INVOICES: " + err));
      } else {
  ...
  this._setXML("invoice.sendByEmail", options, function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT SEND INVOICE BY EMAIL: " + err);
      } else {
        return self;
      }
    });
  ...
  this._setXML("invoice.sendBySnailMail", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT SEND INVOICE BY SNAIL MAIL: " + err));
      } else {
        return self;
      }
    });
  ...
  this._setXML("invoice.lines.add", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        ...
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        return new Error("CANNOT ADD LINE ITEM TO INVOICE: " + err));
      } else {
        self.get(self.invoice_id, fn);
      }
    });
