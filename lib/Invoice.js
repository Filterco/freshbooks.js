...
Invoice.prototype._setXML = function(method, fn) {
  ...
  switch(method) {
    ...
    case "invoice.sendByEmail":
      ...
      console.log("self.invoice_id", self.invoice_id);
    break;
 ...
 * @api public
 */
 Invoice.prototype.getPdf = function(fn) {
  var self = this
    , options = [];
  ...
  this._setXML("invoice.getPdf", options, function(xml) {
    // console.log("XML", xml);
    self.freshbooks._getData(xml, function(err, pdf) {
      // console.log("XML", xml);
      if(null !== err) {
        fn(err);
      } 
      // else if("ok" !== xml.get("//xmlns:response",self.freshbooks.ns).attr("status").value()) {
      //   err = xml.get("//xmlns:error",self.freshbooks.ns).text();
      //   fn(new Error("CANNOT DOWNLOAD INVOICE TO PDF: " + err));
      // }
       else {
        fn(null, self);
      }
